...
exports.last = function(req, res, next) {
  if (res.locals.bundle) {
    var output = toObject(res.locals.bundle);
    if (req.body.format === 'js') {
      return res.send(output);
    } else if (req.body.format === 'html' || req.query.format === 'html') {
      return res.render(this.templateRoot + '/' + req.templatePath, output);
    } else {
      return res.status(res.locals.status_code).json(output);
    }
  }
  res.send();
};
...
/**
 * Realizes any virtual getters in a Mongoose object (or list of objects).
 */
function toObject(docs) {
  if(util.isArray(docs))
    return docs.map(function(doc) { return doc.toObject(); })
